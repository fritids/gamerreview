<?php

	/*
		Template Name: Sign Up Page
	*/


?>

<?php get_header(); ?>

 <div id='main-content'>

 	<h1>Why Sign Up?</h1>
	
	<p id='signfirst'>
		So by now you have seen some of our site and what we are about here. You should have plenty of reasons to sign up! If you still need some convincing, check out the reasons why you shoyld sign up here. The more the merrier here at Gamerreview!
	</p>
	<ul id='reasons'>
		<li>Community full of gamers and writers to share your work with!</li>
		<li>Get to posting your reviews for others to see right away!</li>
		<li>Create a profile with information about yourself!</li>
	</ul>
	<p><i>
		"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed tempus pharetra lectus id tincidunt. Nullam viverra ante vitae quam ornare, nec tempor tellus cursus. Vestibulum imperdiet turpis augue, sed volutpat nulla pretium varius. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus."</i> - Name
	</p>



 </div>

 <?php get_sidebar('signup'); ?>

 <?php get_footer(); ?>